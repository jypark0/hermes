...
class CHPDEonMesh(InMemoryDataset):
    ...
    def _read_data(self, path):
        # Load mesh
        pos = np.load(f"{path}_vertices.npy").astype(np.float32)
        pos = torch.from_numpy(pos)
        face = np.load(f"{path}_faces.npy").T.astype(np.float32)
        face = torch.from_numpy(face)

        # Load values
        values = np.load(f"{path}_values.npy").astype(np.float32)
        c = torch.from_numpy(values).permute(1, 0).contiguous()

        return Data(pos=pos, face=face, c=c)

    def len(self) -> int:
        # Num meshes * num_samples * num_time_windows
        n = self._data.mesh_idx.shape[0] * (
            self._data.c.shape[-1] - self.input_length - self.output_length + 1
        )
        return n
    ...
    def get(self, idx: int) -> Data:
        ...
        t_per_sample = (
            self._data.c.shape[-1] - self.output_length - self.input_length + 1
        )
